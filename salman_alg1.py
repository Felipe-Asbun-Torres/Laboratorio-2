...
df = pd.read_csv("Modelo3_Ben_arreglado.txt", sep=';')

# Normalizar nombres de columnas y preparar
df.columns = df.columns.str.lower().str.strip()
df = df.rename(columns={'densidad': 'density', 'ton': 'tonnage'})

# Crear columnas necesarias para compatibilidad
df['material'] = np.where(df['tonnage'] > 0, 1, 8)  # mineral o estéril
df['cueq'] = df['cus']  # Aquí puedes poner fórmula real de equivalencia si es necesario
df['cpy'] = 0  # placeholder si se usa en features
df['au'] = 0  # placeholder si se usa en features
df['tipomineral'] = 'default'  # placeholder si no hay litología real
...
MAX_CLUSTER_SIZE = 40
MIN_CLUSTER_SIZE = 10
r = 0.4  # penalización litológica
MAX_ITERATIONS = 1

df["spatial_cluster"] = -1
global_cluster_id = 0
bancos = np.sort(df['z'].unique())

start_time = time.time()

for z in bancos:
    banco_df = df[df['z'] == z].copy().sort_values(by='x')
    ...
    for k in range(2, min(Cmax, len(banco_mineral))):
        ...
        scores[k] = silhouette_score(banco_mineral[features], kmeans.labels_)
    ...
    banco_df.loc[banco_df["material"] == 8, "cluster"] = best_k
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        membership_cols = membership_df.columns.tolist()
        ...
        for i in banco_df.index:
            if banco_df.loc[i, 'material'] == 8: continue
            for u in get_neighbors(i, banco_df):
                if banco_df.loc[u, 'material'] == 8: continue
        ...
        for block in banco_df.index:
            ...
            visited.add(block)
            for n in get_neighbors(block, banco_df):
                if len(current_cluster) >= MAX_CLUSTER_SIZE: break
        ...
        for cid, members in list(clusters.items()):
            if len(members) < MIN_CLUSTER_SIZE:
                ...
                neighbors = set(n for sub in neighbors for n in sub if n in block_cluster) - set(members)
                best_match = None
                min_gap = float('inf')
                for n in neighbors:
        ...
        for block in banco_df.index:
            ...
            if len(same_cluster_neighbors) == 1:
                ...
                if neighbor_clusters:
                    ...
                    if neighbor_clusters[best_cid] >= 2:
        ...
        for cid in cluster_ids:
            ...
            if destinos.sum() > 0: dh_list.append(destinos.max() / destinos.sum())
            if tipos.sum() > 0: ru_list.append(tipos.max() / tipos.sum())
